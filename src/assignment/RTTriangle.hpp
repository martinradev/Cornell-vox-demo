...
namespace FW {
	...
	struct RTTriangle {
		...
		inline float minComponent(int i) const {
			return FW::min(m_vertices[0].p[i], m_vertices[1].p[i], m_vertices[2].p[i]);
		}

		inline float maxComponent(int i) const {
			return FW::max(m_vertices[0].p[i], m_vertices[1].p[i], m_vertices[2].p[i]);
		}
